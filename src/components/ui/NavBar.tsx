...
export default function NavBar() {
  ...
  return (
    <>
      <nav className='fixed px-10 sm:px-15 py-5 top-0 left-0 right-0 flex items-center justify-between text-text bg-background border-b border-primary transition-colors z-50'>
      ...
      <ul className={`fixed sm:hidden top-0 right-0 bottom-0 h-dvh w-dvw bg-background text-text flex flex-col justify-center items-center gap-10 text-xl font-semibold transition-transform duration-500 ${menuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <Navigation />
      </ul>
    </>
  )
